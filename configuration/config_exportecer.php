...
<?php if($forward == 'transaksibeli'){ ?>
      <table class="table">
                      ...
                      <?php
        error_reporting(E_ALL ^ (E_NOTICE | E_WARNING));
         $query1="select * from $forward where nota='$search' order by no";
        $hasil = mysqli_query($conn,$query1);
        $no = 1;
        while ($fill = mysqli_fetch_assoc($hasil)){
...
<?php } ?> 
 ...
 <?php if($forward == 'transaksimasukeceran'){ ?>
      <table class="table">
                                        <thead>
                                            <tr>
                                              <th>No</th>
...
<?php if($forward == 'revenue'){
  ...
  ?>
	...
	error_reporting(E_ALL ^ (E_NOTICE | E_WARNING));
if($tahun == null || $tahun == ""){
  $query1="SELECT * FROM  $forward where nota IN (SELECT nota FROM transaksimasukeceran) order by no ";
}else{
  $query1="SELECT * FROM  $forward where nota IN (SELECT nota FROM transaksimasukeceran) and tglbayar like '$tahun-$bulan-%' order by no ";
}
				$hasil = mysqli_query($conn,$query1);
				$no = 1;
				while ($fill = mysqli_fetch_assoc($hasil)){
